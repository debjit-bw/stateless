...
use reth_ethereum_primitives::{Block, EthPrimitives, EthereumReceipt, TransactionSigned};
use reth_evm::{
    ConfigureEvm, Evm,
    block::BlockExecutor,
    execute::{BlockExecutionOutput, Executor},
};
use reth_primitives_traits::{RecoveredBlock, SealedHeader, logs_bloom};
    ...
    let (transaction_state_checkpoints, result) = {
        ...
        for (transaction_index, transaction) in current_block.transactions_recovered().enumerate() {
            ...
            if transaction_indices.get(checkpoints.len()) == Some(&transaction_index) {
                ...
                let block_hash = candidate_block_hash(
...
mod tests {
    ...
    use reth_ethereum_primitives::{EthereumReceipt, Transaction, TransactionSigned};
    ...
    fn every_prefix_varying_field_changes_the_candidate() {
        ...
        let base = candidate_block_hash(&settling, &txs[..1], &receipts[..1], root).unwrap();

        // transactions_root
        assert_ne!(
            base,
            candidate_block_hash(&settling, &txs[..2], &receipts[..1], root).unwrap(),
            "a longer transaction prefix must seal a different candidate",
        );
        // receipts_root, logs_bloom and gas_used all move together here
        assert_ne!(
            base,
            candidate_block_hash(&settling, &txs[..1], &receipts[..2], root).unwrap(),
            "a longer receipt prefix must seal a different candidate",
        );
        // state_root
        assert_ne!(
            base,
            candidate_block_hash(&settling, &txs[..1], &receipts[..1], B256::repeat_byte(0xcd))
                .unwrap(),
    ...
    fn the_candidate_header_is_exactly_this() {
        ...
        assert_eq!(
            candidate_block_hash(&settling, &txs, &receipts, root).unwrap(),
            expected.hash_slow(),
        );
    }
    ...
    fn unreproducible_fields_are_refused() {
        ...
        assert!(matches!(
            candidate_block_hash(&with_bal, &[], &[], B256::ZERO),
        ...
        assert!(matches!(
            candidate_block_hash(&with_requests, &[], &[], B256::ZERO),
        ...
        assert!(matches!(
            candidate_block_hash(&with_blobs, &[], &[], B256::ZERO),
        ...
        assert!(candidate_block_hash(&cancun, &[], &[], B256::ZERO).is_ok());
    }
